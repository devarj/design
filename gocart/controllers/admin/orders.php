...
class Orders extends Admin_Controller {	
	...
	function view($id)
	{
        ...
        $data['freebies'] = $this->Product_model->get_order_freebies($id);
	...
	//edit
	function packing_slip2($order_id)
	{
		$this->load->helper('date');
		$data['order']		= $this->Order_model->get_order($order_id);
		
		$this->load->view($this->config->item('admin_folder').'/packing_slip2.php', $data);
	}
	...
	function edit_status()
    {
    	...
    	$order['payment_status']	= $this->input->post('payment_status');
        if($order['payment_status'] == 'Paid'){
			
			$orderx		= $this->Order_model->get_order($order['id']);
			
			if($orderx->customqty > 0){
				$where['itemid'] = 3;
				$stocks = $this->Common_model->get_where_orderby('stocks', $where, 1, 'poid', 'ASC');
				
				$selectedStocks = $stocks[0]->qty;
				$save['dispense'] = $orderx->customqty;
				$this->Common_model->update('stocks', $save, $where);
			}
			else{
				$contents = $orderx->contents;
			
				foreach($contents as $content){
					
					$where['itemid'] = $content['stock_id'];
					$stocks = $this->Common_model->get_where_orderby('stocks', $where, 1, 'poid', 'ASC');
					
					$selectedStocks = $stocks[0]->qty;
					$save['dispense'] = $content['quantity'];
					$this->Common_model->update('stocks', $save, $where);
				}
			}
			
			//die();
            //$this->order_confirmation($order['id']);
        }
		
		if($order['payment_status'] == 'Cancelled'){
			$orderx		= $this->Order_model->get_order($order['id']);
			
			/* if($orderx->tnumber != null){
    	...
    	echo url_title($order['payment_status']);
    }
	
	function down_amount($id){
		$save['down_amount'] = $this->input->post('down_amount');
		$where['id'] = $id;
	...
	function edit_dstatus()
    {
    	$this->auth->is_logged_in();
    	$order['id']                = $this->input->post('id');
    	$order['delivery_status']	= $this->input->post('delivery_status');
    	
    	$this->Order_model->save_order($order);
    	
    	echo url_title($order['delivery_status']);
    }
    ...
    function order_confirmation($order_id)
    {
        ...
        // {order_summary}
        $row['content'] = str_replace('{order_summary}', $this->load->view($this->config->item('admin_folder').'/order_email', $data, true), $row['content']);
