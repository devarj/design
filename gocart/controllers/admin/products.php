...
class Products extends Admin_Controller {	
	...
	function product_autocomplete()
	{
		...
		{
			$results	= $this->Product_model->product_autocomplete($name, $limit, $id);
	...
	function form($id = false, $duplicate = false)
	{
		...
		$data['itemtype'] = $this->Common_model->get('raw');
		//$data['categories']		= $this->Category_model->get_categories_tierd();
		...
		{	
			...
			$wherex['itemid'] = $product->stock_id;
			
			$datax = $this->Common_model->get_where('stocks', $wherex, 1);
		...
		$this->form_validation->set_rules('price', 'lang:price', 'trim|required|numeric|floatval');
		$this->form_validation->set_rules('saleprice', 'lang:saleprice', 'trim|numeric|floatval');
		$this->form_validation->set_rules('unit', 'lang:unit', 'trim|max_length[50]');
		...
		*/
		...
		else
		{
			...
			$save['stock_id']			= $this->input->post('stock_id');
	...
	function getstockvalue($id){
		
		$where['itemid'] = $id;
		
		$data = $this->Common_model->get_where('stocks', $where, 1);
