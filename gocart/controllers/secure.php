...
//        echo 'test';
	...
	function newsupplier(){
		$data['page_title'] = 'Register as New Supplier';
		$this->load->view('newsupplier', $data);
	}
	...
	function register_supplier(){
		if($this->input->post('') != $this->input->post('')){
			...
			$save['permit'] = $this->input->post('permit');
    ...
    function register() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // get the coupon code
            $res_code = $this->db->where('customer_id', $id)->get('coupons');
            $row_code = $res_code->row_array();
...
//            
//            // {coupon_code}
//            $row['content'] = str_replace('{coupon_code}', $row_code['code'], $row['content']);
//            $row['content'] = str_replace('{coupon_amount}', $this->config->item('coupon_reg_amount'), $row['content']);
//            $row['content'] = str_replace('{coupon_minimum_transaction}', $this->config->item('coupon_minimum_transaction'), $row['content']);
            ...
            // {register_summary}
            $row['content'] = str_replace('{register_summary}', $this->load->view('register_email', $data, true), $row['content']);

            // {coupon_code}
            $row['content'] = str_replace('{coupon_code}', $row_code['code'], $this->load->view('register_email', $data, true));
...
//            print_r($row['content']); die();
    ...
    function my_account($offset=0) {
        ...
        $data['orders2'] = $this->order_model->get_customer_orders2($this->customer['id']);

		//debug($data['orders2'], 1);
    ...
    function address_form($id = 0) {
        ...
        if ($id == 0) {
            ...
            $data['city_menu'] = $this->location_model->get_cities_menu(array_shift(array_keys($data['countries_menu'])));
        } else {
            $data['zones_menu'] = $this->location_model->get_zones_menu($data['country_id']);
            $data['city_menu'] = $this->location_model->get_cities_menu($data['country_id']);
        }
	...
	function submit_tnumber(){
		$id = $this->input->post('id');
		$tnumber = $this->input->post('tnumber');
		
		//$where1['id'] = $id;
		//$data = $this->Common_model->get_where('order_items', $where1, 0);
		
		$where['id'] = $id;
		$save['tnumber'] = $tnumber;
		$this->Common_model->update('orders', $save, $where); 
