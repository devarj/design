...
<div class="page-header" id="anchor">
    <h1><?php echo 'Add Address in Auto Shipment'; ?></h1>
</div>
...
{
	$.post("<?php echo site_url('ship/address_form');?>/"+$('#f_id').val(), {	company: $('#f_company').val(),
		...
		function(data){
			if(data == 1)
			{
				window.location = "<?php echo site_url('ship/view_all');?>";
			}
			else
			{
