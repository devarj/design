...
class go_cart
{
    ...
    private function _init_properties($totals_only = false, $preserve_customer = false)
    {
        ...
        $this->_cart_contents['reward_discount'] = 0;
        $this->_cart_contents['stock_id'] = 0;
        // tax
...
//            $this->_cart_contents['items'][$cartkey]['quantity'] = ceil($quantity);
            $this->_cart_contents['items'][$cartkey]['quantity'] = $quantity;
        }
        else
        {
...
/** start jerome.j reward points 10-01-2014 */
       private function _insert_reward($coupon_code)
    {
        ...
        if ($coupon)
        {
            ...
            if (!isset($this->_cart_contents['coupon_list'][$coupon_code]))
            {
                ...
                if ($this->CI->Coupon_model->is_valid($coupon))
                {
                    ...
                    {  // Otherwise, this is a product level discount
...
/** end jerome.j reward points 10-01-2014 */
    private function _insert_coupon($coupon_code)
    {
        ...
        $customer_id = $this->_cart_contents['customer']['id'];

        $coupon = $this->CI->Coupon_model->get_coupon_by_code($coupon_code);
        
        $coupon_customer = $this->CI->Coupon_model->get_coupon_code_by_customer($coupon_code, $customer_id);
        ...
        if ($coupon)
        {
            if ($coupon_customer)
            {
                ...
                if (!isset($this->_cart_contents['coupon_list'][$coupon_code]))
                {
                    ...
                    if ($this->CI->Coupon_model->is_valid($coupon))
                    {
                        if ($this->_cart_contents['cart_subtotal'] < $this->CI->config->item('coupon_minimum_transaction'))
                        {
                            $is_applied = false;
                            $is_minimum = true;
                        }
                        ...
                        if (!$is_applied)
                        {
                            if ($is_minimum)
                            {
                                // message coupon minimum single transaction
                                return array('error' => lang('coupon_minimum_amount'));
                            }
                            else
                            {
                            // message coupon added but not applied
    ...
    private function _calculate_coupon_discount()
    {
        ...
        $click_Reward_button =  $this->CI->session->userdata('click_rewards_points');
        if ($click_Reward_button)
        {
            $this->_cart_contents['reward_discount'] = $this->CI->session->userdata('select_rewards_points');
        }
    ...
    private function _calculate_gift_card_discount()
    {
        ...
        foreach ($this->_cart_contents['gc_list'] as &$card)
        {
            ...
            if ($card['balance'] >= $this->_cart_contents['cart_total'])
            { // cart total should be calculated already 
    ...
    private function _save_cart($recalculate = true)
    {
        ...
        if ($recalculate)
        {
            ...
            $this->_cart_contents['cart_total'] = ($total - $this->_cart_contents['coupon_discount']) - $this->_cart_contents['reward_discount']; // $this->_cart_contents['group_discount'];
    ...
    function update_cart($qty_list = false, $coupon_code = false, $gc_code = false,$reward = false)
    {
...
//have to run an isset here due to an error in IE (presumeably due to self signed certificate error double loading)
    ...
    function save_order($dc)
    {
        ...
        if ($none_shippable)
        {
            $save['payment_status'] = $this->CI->config->item('nonship_status');
        }
        else
        {
            $save['payment_status'] = $this->CI->config->item('order_status');
        }
        ...
        $deliverydate = date('Y-m-d', strtotime($bill['deliverydate']));
        $deliverytime = date('H:i:s', strtotime($bill['deliverytime']));
        $save['shipped_on'] = $deliverydate.' '.$deliverytime;

        //shipping information
        $delivery_charge = ($this->CI->config->item('allow_delivery_charge')) ? $this->CI->config->item('delivery_amount') : 0;
		
        $save['shipping_method'] = $this->_cart_contents['shipping']['method'];
        $save['shipping'] = $this->_cart_contents['shipping']['price']+$dc;
        ...
        $save['reward_points'] = $this->_cart_contents['reward_discount'];
        $save['subtotal'] = $this->_cart_contents['cart_subtotal'];
        $save['total'] = $this->_cart_contents['cart_total']+$save['shipping'];
    ...
    function reward_discount()
    {
        return $this->_cart_contents['reward_discount'];
    }
    ...
    function destroy($keep_customer_data = true)
    {
        ...
        $this->CI->session->unset_userdata('click_rewards_points');
