...
<table class="table table-striped">
    <thead>
		<tr>
			...
			<th><?php echo sort_url('payment_status','payment_status', $sort_by, $sort_order, $code, $this->config->item('admin_folder')); ?></th>
    ...
    <?php if($this->auth->check_access('Finance') || $this->auth->check_access('Admin')):?>
		<td style="span2">
			<?php echo form_dropdown('payment_status', $this->config->item('payment_statuses'), $order->payment_status, 'id="status_form_'.$order->id.'" class="span2" style="float:left;"'); ?>
    ...
    <?php else:?>
        <td style="span2"><?php echo $order->payment_status; ?></td>
    <?php endif;?>
		<td><div class="MainTableNotes"><?php echo format_currency($order->total); ?></div></td>
		<td><div class="MainTableNotes"><?php echo $order->tnumber; ?></div></td>
		<td>
...
{
	show_animation();
	$.post("<?php echo site_url($this->config->item('admin_folder').'/orders/edit_status'); ?>", { id: id, payment_status: $('#status_form_'+id).val()}, function(data){
		setTimeout('hide_animation()', 500);
	});
}

function save_dstatus(id)
{
	show_animation();
	$.post("<?php echo site_url($this->config->item('admin_folder').'/orders/edit_dstatus'); ?>", { id: id, delivery_status: $('#dstatus_form_'+id).val()}, function(data){
		setTimeout('hide_animation()', 500);
	});
}
