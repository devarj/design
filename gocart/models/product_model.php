...
Class Product_model extends CI_Model
{
	...
	function product_autocomplete($name, $limit, $id)
	{
		return	$this->db->like('name', $name)->where('id !=', $id)->get('products', $limit)->result();
	}
	
	function search_autocomplete($name, $limit)
	{
		return	$this->db->like('name', $name, 'after')->where('enabled', 1)->get('products', $limit)->result();
	}
	...
	function get_freebie($amount)
        {
                $this->db->where('amount <=', $amount);
                $this->db->where('status =', 1);
                $this->db->order_by("amount", "DESC");
                $this->db->limit(1);
                return $this->db->get('freebies')->row();
        }
        function order_freebies($data){
                $this->db->insert('order_freebies', $data);
        }
        function get_order_freebies($id){
                $this->db->select('order_freebies.freebie');
                $this->db->join('orders', 'order_freebies.order_id = orders.order_number');
                $this->db->where('orders.id =', $id);
                return $this->db->get('order_freebies')->row();
        }
	function get_product($id, $related=true)
	{
		...
		if(!empty($related))
		{
			...
			$result->related_products	= $this->db->where('enabled', 1)->get('products')->result();
		}
		else
		{
	...
	function get_cart_ready_product($id, $quantity=false)
	{
        ...
        function get_products_all( $limit = false, $offset = false, $by=false, $sort=false)
	{
		//if we are provided a category_id, then get products according to category
		
			$this->db->select('category_products.*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false)->from('category_products')->join('products', 'category_products.product_id=products.id')->where(array('enabled'=>1));
        ...
        function count_all_products_enable()
	{
//		return $this->db->select('product_id')->from('category_products')->join('products', 'category_products.product_id=products.id')->where('products.enabled',1)->count_all_results();
		$test = $this->db->select('product_id')->from('category_products')->join('products', 'category_products.product_id=products.id')
                        ->where('products.enabled',1)->where('weight !=', 0)->group_by('products.id')->get()->result();
                return count($test);
	}
    function get_featured_products($limit = false)
	{
