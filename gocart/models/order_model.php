...
Class order_model extends CI_Model
{
		...
		if ($search)
		{
			if(!empty($search->term))
			{
				...
				foreach($term as $t)
				{
					...
					$like	.= $operator." `payment_status` ".$not."LIKE '%".$t."%' ";
	...
	function get_customer_orders2($id)
	{
		$this->db->group_by('orders.id');
		$this->db->order_by('ordered_on', 'DESC');
		return $this->db->get_where('orders', array('customer_id'=>$id), 15, 0)->result();
	}
	...
	function get_order_number($ubiz_reference_id)
	{
		$this->db->where('ubiz_reference_id', $ubiz_reference_id);
		$result = $this->db->get('orders');
	...
	function update_content($oid, $content){
		$this->db->where('order_id', $oid)->delete('order_items');
		foreach($content as $ct){
			$save['contents'] = serialize($ct);
			
			$save['product_id'] = $ct['id'];
			$save['quantity'] 	= $ct['quantity'];
			$save['order_id']	= $oid;
	...
	function save_order($data, $contents = false)
	{
		...
		else
		{
			...
			$order	= array('order_number'=> 'IDSGN'.date('U').$id);
	...
	function get_best_sellers($start, $end)
	{
		...
		{
			$product				= $this->db->where('id', $key)->where('enabled', '1')->get('products')->row();
			if($product)
			{
    ...
    function get_best_sellers_home($start, $end)
	{
		if(!empty($start))
		{
			$this->db->where('ordered_on >=', $start);
    ...
    function get_invoices($start, $end)
    {
        if (!empty($start))
        {
            $this->db->where("DATE_FORMAT(`invoice_date`, '%m/%d/%Y') >=", $start);
        }
        if (!empty($end))
        {
            $this->db->where("DATE_FORMAT(`invoice_date`, '%m/%d/%Y') <", $end);
        }
        $this->db->select('invoice_date');
        $this->db->select('invoice_no');
        $this->db->select('order_number');
        $this->db->select('ordered_on');
        $this->db->select('waybill_no');
        $this->db->select('subtotal');
        $this->db->select('(subtotal * 0.05) as transaction_fee');
        $this->db->select('(subtotal - (subtotal * 0.05)) as net_amount');
